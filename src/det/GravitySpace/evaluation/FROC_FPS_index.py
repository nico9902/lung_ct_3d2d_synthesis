...
from src.det.GravitySpace.metrics.utility.my_round_value import my_round_value
from src.det.GravitySpace.plot.coords.read_coords import read_coords
...
def FROC_FPS_index(detections_path: str,
    ...
    index = 0
    for n in range(FPS_test.shape[0]):
        if FPS_test[n] >= FPS:
            index = n
            break
