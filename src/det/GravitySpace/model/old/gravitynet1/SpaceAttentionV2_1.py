...
class SpaceAttentionV2_1(nn.Module):
    """
    SpaceAttentionV2_1
    """
    ...
    def __init__(self,
        ...
        """
        super(SpaceAttentionV2_1, self).__init__()
    ...
    def forward(self, window_features: torch.Tensor,
        ...
        # Attention #
        # --------- #
        # key enhance the central slice window
        V = V + V[:, current_slice_index, :].unsqueeze(1)  # B x K x hidden_dim
        # query and key are the entire window
        ...
        # sum along K and remove the window dimension
        attention_output = attention_output.sum(dim=1).squeeze(1)  # B x F x H_FM x W_FM
