...
def generate_nodule_mask(patient_id, volume_shape, z_coords, consensus_threshold=1):
    ...
    for nodule_cluster in scan.cluster_annotations():
        ...
        for scan_z_idx in range(z_slice.start, z_slice.stop):
            ...
            mask[volume_z_idx, y_slice, x_slice] = np.logical_or(
                mask[volume_z_idx, y_slice, x_slice],
                cmask[:, :, local_z_idx],
            )
...
def process_lidc_case(
    ...
    if write_mask:
        ...
        mask_array = generate_nodule_mask(patient_id, full_shape, z_coords, consensus_threshold)
        if mask_array is not None:
            mask_image = sitk.GetImageFromArray(mask_array)
            mask_image.CopyInformation(full_image)
            ...
            label_rows = labels_from_mask_image(patient_id, mask_image)
