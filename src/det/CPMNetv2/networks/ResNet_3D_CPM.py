...
from src.det.CPMNetv2.utils.box_utils import nms_3D
...
class Detection_loss(nn.Module):
    def __init__(
        ...
        topk=7,
        spacing=[2.0, 1.0, 1.0],
        debug_target_stats=False,
        debug_target_stats_interval=50,
    ):
        super(Detection_loss, self).__init__()
        self.crop_size = crop_size
        self.topk = topk
        self.spacing = np.array(spacing)
        self.debug_target_stats = debug_target_stats
        self.debug_target_stats_interval = debug_target_stats_interval
        self._debug_target_stats_calls = 0

    def _maybe_print_target_debug(self, annotations, target_scores):
        if not self.debug_target_stats:
            return
        if self.debug_target_stats_interval > 0 and self._debug_target_stats_calls % self.debug_target_stats_interval != 0:
            self._debug_target_stats_calls += 1
            return
        self._debug_target_stats_calls += 1

        all_cls = annotations[:, :, -1].detach().cpu()
        target_cls = target_scores.detach().squeeze(-1).cpu()
        all_cls_unique = torch.unique(all_cls, return_counts=True)
        target_cls_unique = torch.unique(target_cls, return_counts=True)
        print("all_cls unique:", (all_cls_unique[0].numpy(), all_cls_unique[1].numpy()))
        print("num positives in target:", int((target_cls > 0).sum().item()))
        print("target_cls unique:", target_cls_unique)
    ...
    def forward(self, output, annotations, device):
        ...
        self._maybe_print_target_debug(annotations, target_scores)
        # merge mask ignore
