...
class RandomTranspose(AbstractTransform):
    ...
    def __call__(self, sample):
        ...
        if len(transpose_list) > 0:
            ...
            rad_t = sample['rad'].copy() if 'rad' in sample and sample['rad'].size > 0 else None
            if rad_t is not None:
                rad_t = rad_t.reshape(-1, 3)
            ...
            for transpose in transpose_list:
                ...
                if rad_t is not None:
                    temp = rad_t.copy()
                    rad_t[:, 0] = temp[:, transpose[1] - 1]
                    rad_t[:, 1] = temp[:, transpose[2] - 1]
                    rad_t[:, 2] = temp[:, transpose[3] - 1]
