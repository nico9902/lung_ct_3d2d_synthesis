...
from .feature_model import FeatureAttentionClassifier


class FeatureAttentionLightningModule(pl.LightningModule):
    def __init__(
        self,
        feature_dim: int,
        ...
        self.model = FeatureAttentionClassifier(feature_dim=feature_dim, dropout=dropout)
    ...
    def forward(self, features: torch.Tensor, valid_mask: torch.Tensor):
        return self.model(features, valid_mask)

    def _shared_step(self, batch, stage: str):
        features, valid_mask, labels, sample_ids = batch
        logits, attention_weights = self(features, valid_mask)
        loss = self.criterion(logits, labels.float())
        scores = torch.sigmoid(logits)
        predictions = (scores >= 0.5).long()
        self.log(f"{stage}_loss", loss, on_epoch=True, prog_bar=True, batch_size=labels.shape[0])
        output = {
    ...
    def _compute_epoch_metrics(self, outputs, stage: str):
        ...
        self.log(f"{stage}_mcc", mcc, prog_bar=True)
