...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/luna16_synthetic_2d_top7_minprob0.3_rbf}"
SYNTHETIC_IMAGES_DIR="${SYNTHETIC_IMAGES_DIR:-data/luna16_saliency_synthetic_detector_top7_minprob0.3_rbf}"
...
WANDB="${WANDB:-1}"
WANDB_PROJECT="${WANDB_PROJECT:-luna16-synthetic-2d-detector-top7-minprob0.3-rbf}"
...
for FOLD in ${FOLDS}; do
  for BACKBONE in ${BACKBONES}; do
    WANDB_RUN_GROUP="${BACKBONE}" python3 -m src.luna16_synthetic_2d.train \
      ...
      --monitor "${MONITOR}" \
      --wandb-group "${BACKBONE}_top7_minprob0.3" \
      "${EXTRA_ARGS[@]}"  
  done
done
