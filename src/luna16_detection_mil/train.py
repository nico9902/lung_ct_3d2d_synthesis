...
from src.luna16_detection_mil.datamodule import DetectionMILDataModule
from src.luna16_detection_mil.lightning_model import DetectionMILLightningModule
from src.luna16_synthetic_2d.models import SUPPORTED_BACKBONES


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LUNA16 detector-crop MIL patient classifier.")
    parser.add_argument("--output-dir", default="outputs/luna16_detection_mil")
    ...
    parser.add_argument("--pooling", default="attention", choices=["mean", "max", "attention"])
    ...
    parser.add_argument("--wandb-project", default="luna16-detection-mil")
...
def build_loggers(args: argparse.Namespace, output_dir: Path):
    loggers = [CSVLogger(save_dir=output_dir.parent, name=args.pooling)]
    if args.wandb:
        import wandb
        from pytorch_lightning.loggers import WandbLogger

        name = args.wandb_name or f"fold_{args.fold}_{args.backbone}_{args.pooling}"
        group = args.wandb_group or f"{args.backbone}_{args.pooling}"
        ...
        run = wandb.init(
            ...
            tags=[args.backbone, args.pooling, f"fold_{args.fold}", "detection_mil"],
...
def save_test_predictions(model: DetectionMILLightningModule, output_dir: Path, args: argparse.Namespace) -> None:
    rows = getattr(model, "test_prediction_rows", [])
    if not rows:
        return
    predictions = pd.DataFrame(rows)
    predictions.insert(0, "pooling", args.pooling)
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir) / f"fold_{args.fold}" / args.pooling
    output_dir.mkdir(parents=True, exist_ok=True)
    with (output_dir / "config.json").open("w") as handle:
        json.dump(vars(args), handle, indent=2)

    datamodule = DetectionMILDataModule(
    ...
    if args.eval_only:
        model = DetectionMILLightningModule.load_from_checkpoint(args.eval_only, pretrained=False)
        test_results = trainer.test(model=model, datamodule=datamodule)
        save_test_metrics(test_results, output_dir)
        save_test_predictions(model, output_dir, args)
        return

    model = DetectionMILLightningModule(
