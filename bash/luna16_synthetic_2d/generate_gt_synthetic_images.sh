#!/usr/bin/env bash
# Generate ground-truth-guided synthetic 2D images: the annotated nodule
# centers and contours (not detector candidates) are the control points of the
# surface. Their control points feed build_nodule_position_distribution.sh.
# Defaults reproduce the run in data/synthetic_2d/luna16_saliency_synthetic_gt
# (see its hydra/fold0 overrides). A single fold CSV
# covers every patient, because train, val and test scans are all written.
set -euo pipefail
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/luna16_saliency_synthetic_gt}"
...
for FOLD in ${GT_FOLDS}; do
  ...
  python -m src.luna16_synthetic_2d.saliency \
    --config-path "${CONFIG_DIR}" \
    --config-name config \
    data.dataset_type=luna16_nii \
    data.processed_dir="${DATA_ROOT}" \
    ...
    saliency.surface_method="${SURFACE_METHOD}" \
    saliency.rbf_smooth="${RBF_SMOOTH}" \
    saliency.shepard_power="${SHEPARD_POWER}" \
