#!/usr/bin/env bash
# Train the 2D backbones on one set of synthetic images across all folds and
# export a per-backbone summary. Also reused by the non-adaptive 2D baselines
# (bash/luna16_2d_baselines/). Extra arguments are forwarded to
# src.luna16_synthetic_2d.train.
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/../common.sh"

EXPERIMENT_NAME="${EXPERIMENT_NAME:?Set EXPERIMENT_NAME, e.g. cpmnetv2_bf16_top4_minprob0.50_rbf}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/luna16_synthetic_2d_${EXPERIMENT_NAME}}"
SYNTHETIC_IMAGES_DIR="${SYNTHETIC_IMAGES_DIR:-${SYNTHETIC_ROOT}/luna16_saliency_synthetic_detector_${EXPERIMENT_NAME}}"
...
echo "Training LUNA16 synthetic 2D experiment"
...
for FOLD in ${FOLDS}; do
  for BACKBONE in ${BACKBONES}; do
    WANDB_RUN_GROUP="${BACKBONE}_${WANDB_GROUP_SUFFIX}" python -m src.luna16_synthetic_2d.train \
...
if [[ "${EXPORT_BACKBONE_SUMMARY}" == "1" ]]; then
  python -m src.luna16_synthetic_2d.export_backbone_summary \
    --output-dir "${OUTPUT_DIR}" \
