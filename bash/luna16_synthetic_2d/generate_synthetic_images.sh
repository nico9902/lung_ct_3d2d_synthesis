#!/usr/bin/env bash
# Generate detector-guided synthetic 2D images for every fold.
#
# CPMNetv2 candidates (top-k above a probability threshold) become control
# points of an adaptive surface (SURFACE_METHOD=rbf|shepard) that is sampled
# through the CT volume. CONTROL_POINT_MODE=fixed|random gives the geometric
# controls used in the ablation. Extra arguments are forwarded to
# src.luna16_synthetic_2d.detector_saliency.
set -euo pipefail
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-top${TOP_K}_minprob${MIN_PROBABILITY}_${SURFACE_METHOD}}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${SYNTHETIC_ROOT}/luna16_saliency_synthetic_detector_${EXPERIMENT_NAME}}"
...
for FOLD in ${FOLDS}; do
  ...
  python -m src.luna16_synthetic_2d.detector_saliency \
    --fold "${FOLD}" \
    --split "${SPLIT}" \
    --csv-file "${CSV_FILE}" \
