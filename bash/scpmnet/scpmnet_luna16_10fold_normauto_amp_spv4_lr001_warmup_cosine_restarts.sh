...
export OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/scpmnet_luna16_10fold_normauto_amp_spv4_lr001_randomcrop_gradclip100_warmup_cosine_restarts_valstart100}"
export EXPERIMENT_PREFIX="${EXPERIMENT_PREFIX:-scpmnet_paper_luna16_normauto_amp_spv4_lr001_gradclip100_warmup_cosine_restarts_valstart100_fold}"
export WANDB_NAME_PREFIX="${WANDB_NAME_PREFIX:-scpmnet_luna16_amp_spv4_lr001_gradclip100_warmup_cosine_restarts_valstart100_fold}"

export PRECISION="${PRECISION:-16-mixed}"
export LR="${LR:-0.01}"
export BATCH_SIZE="${BATCH_SIZE:-24}"
export SAMPLES_PER_VOLUME="${SAMPLES_PER_VOLUME:-4}"
...
export RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)_spv4_lr001_gradclip100_warmup_cosine_restarts_valstart100}"
export LOG_DIR="${LOG_DIR:-logs/scpmnet_luna16_amp_spv4_lr001_gradclip100_warmup_cosine_restarts_valstart100_${RUN_TAG}}"
